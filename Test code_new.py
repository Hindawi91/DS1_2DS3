...
for exp in expirements:   
    ...
    for j,category in enumerate (categories): 
        im_files = glob(f'./brats_syn_256_lambda0.1/results_formatted/UT/{category}/*.j*')
    ...
    for model_name in models_names:
        ...
        y_pred_prob = model.predict_proba(imagesNP)
...
df.to_excel ('./CNN UT - Base Model_Metrics.xlsx', index = False, header=True)
...
df.to_excel ('./CNN UT - Base Model_CMs.xlsx', index = False, header=True)
...
final.to_excel ('./CNN UT - Base Model_CMs2.xlsx', index = False, header=True)
